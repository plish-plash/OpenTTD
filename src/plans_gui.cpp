...
struct PlansWindow : Window {
	...
	PlansWindow(WindowDesc *desc) : Window(desc)
	{
		...
		this->text_offset = WidgetDimensions::scaled.frametext.left + spr_dim.width + 2 + SETTING_BUTTON_WIDTH;
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			case WID_PLN_NEW:
				Command<CMD_ADD_PLAN>::Post(STR_ERROR_CAN_T_ADD_PLAN, CcAddPlan, _local_company);
				break;
			...
			case WID_PLN_DELETE:
				if (this->selected != INT_MAX) {
					if (this->list[this->selected].is_plan) {
						Command<CMD_REMOVE_PLAN>::Post(this->list[this->selected].plan_id);
					} else {
						Command<CMD_REMOVE_PLAN_LINE>::Post(this->list[this->selected].plan_id, this->list[this->selected].line_id);
					}
				}
				break;
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (widget) {
			case WID_PLN_LIST: {
				uint y = r.top + WidgetDimensions::scaled.framerect.top; // Offset from top of widget.
				if (this->vscroll->GetCount() == 0) {
					DrawString(r.left + WidgetDimensions::scaled.frametext.left, r.right - WidgetDimensions::scaled.frametext.right, y, STR_STATION_LIST_NONE);
					return;
				}
				...
				uint text_left  = r.left + (rtl ? WidgetDimensions::scaled.framerect.left : this->text_offset);
				uint text_right = r.right - (rtl ? this->text_offset : WidgetDimensions::scaled.framerect.right);
	...
	{
		this->vscroll->SetCapacityFromWidget(this, WID_PLN_LIST, WidgetDimensions::scaled.framerect.top + WidgetDimensions::scaled.framerect.bottom);
	}
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			case WID_PLN_LIST:
				...
				size->height = resize->height * 5 + WidgetDimensions::scaled.framerect.top + WidgetDimensions::scaled.framerect.bottom;
				break;
		}
	}
	...
	virtual void OnPlaceDrag(ViewportPlaceMethod select_method, ViewportDragDropSelectionProcess select_proc, Point pt)
	{
		...
		if (_current_plan && tile < Map::Size()) {
...
void CcAddPlan(Commands cmd, const CommandCost &result, PlanID new_plan)
{
	...
	_current_plan = Plan::Get(new_plan);
